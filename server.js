...
// Rate limiters - more lenient for production
...
async function getAccessToken() {
  ...
  try {
    ...
    throw new Error("OAuth token fetch failed: " + (err.response?.data?.error || err.message));
  }
}
...
app.get('/api/beatmap/:id', apiLimiter, async (req, res) => {
  ...
  try {
    ...
    if (isNaN(id) || id <= 0) {
      console.log('❌ Invalid beatmap ID:', req.params.id);
    ...
    if (err.response?.status === 401) {
      return res.status(500).json({ error: 'API authentication failed - check credentials' });
    }
...
app.get('/api/test', (req, res) => {
  res.json({
    ...
    has_osu_credentials: !!(client_id && client_secret)
  });
});
